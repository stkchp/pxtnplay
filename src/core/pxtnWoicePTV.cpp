...
static bool _Write_Wave(pxwrDoc *p_doc, const pxtnVOICEUNIT *p_vc, s32 *p_total)
{
	...
	if (!p_doc->v_w(p_vc->type, p_total))
		goto End;

	switch (p_vc->type) {
	// Coodinate (3)
	case pxtnVOICE_Coodinate:
		if (!p_doc->v_w(p_vc->wave.num, p_total))
			goto End;
		if (!p_doc->v_w(p_vc->wave.reso, p_total))
			goto End;
		num = p_vc->wave.num;
		for (i = 0; i < num; i++) {
			uc = (s8)p_vc->wave.points[i].x;
			if (!p_doc->w(&uc, 1, 1))
				goto End;
			(*p_total)++;
			sc = (s8)p_vc->wave.points[i].y;
			if (!p_doc->w(&sc, 1, 1))
				goto End;
			(*p_total)++;
		}
	...
	case pxtnVOICE_Overtone:
		...
		num = p_vc->wave.num;
		for (i = 0; i < num; i++) {
			if (!p_doc->v_w(p_vc->wave.points[i].x, p_total))
				goto End;
			if (!p_doc->v_w(p_vc->wave.points[i].y, p_total))
				goto End;
		}
		break;
	...
	case pxtnVOICE_Sampling:
		if (!p_doc->v_w(p_vc->p_pcm->get_ch(), p_total))
			goto End;
		if (!p_doc->v_w(p_vc->p_pcm->get_bps(), p_total))
			goto End;
		if (!p_doc->v_w(p_vc->p_pcm->get_sps(), p_total))
			goto End;
		if (!p_doc->v_w(p_vc->p_pcm->get_smp_head(), p_total))
			goto End;
		if (!p_doc->v_w(p_vc->p_pcm->get_smp_body(), p_total))
			goto End;
		if (!p_doc->v_w(p_vc->p_pcm->get_smp_tail(), p_total))
			goto End;

		size = p_vc->p_pcm->get_buf_size();

		if (!p_doc->w(p_vc->p_pcm->get_p_buf(), 1, size))
			goto End;
		*p_total += size;
		break;
...
static bool _Write_Envelope(pxwrDoc *p_doc, const pxtnVOICEUNIT *p_vc, s32 *p_total)
{
	...
	// envelope. (5)
	if (!p_doc->v_w(p_vc->envelope.fps, p_total))
		goto End;
	if (!p_doc->v_w(p_vc->envelope.head_num, p_total))
		goto End;
	if (!p_doc->v_w(p_vc->envelope.body_num, p_total))
		goto End;
	if (!p_doc->v_w(p_vc->envelope.tail_num, p_total))
		goto End;

	num = p_vc->envelope.head_num + p_vc->envelope.body_num + p_vc->envelope.tail_num;
	for (i = 0; i < num; i++) {
		if (!p_doc->v_w(p_vc->envelope.points[i].x, p_total))
			goto End;
		if (!p_doc->v_w(p_vc->envelope.points[i].y, p_total))
			goto End;
	}
...
static bool _Read_Wave(pxwrDoc *p_doc, pxtnVOICEUNIT *p_vc)
{
	...
	case pxtnVOICE_Coodinate:
		...
		if (!p_doc->v_r(&p_vc->wave.reso))
			goto End;
		num = p_vc->wave.num;
		if (!_malloc_zero((void **)&p_vc->wave.points, sizeof(sPOINT) * num))
			goto End;
		for (i = 0; i < num; i++) {
			if (!p_doc->r(&uc, 1, 1))
				goto End;
	...
	case pxtnVOICE_Overtone:
		...
		if (!_malloc_zero((void **)&p_vc->wave.points, sizeof(sPOINT) * num))
			goto End;
		for (i = 0; i < num; i++) {
			if (!p_doc->v_r((s32 *)&p_vc->wave.points[i].x))
				goto End;
			if (!p_doc->v_r((s32 *)&p_vc->wave.points[i].y))
				goto End;
		}
		break;
	...
	case pxtnVOICE_Sampling: {
		...
		// size = ( p_vc->pcm.smp_head + p_vc->pcm.smp_body + p_vc->pcm.smp_tail ) * p_vc->pcm.ch *
		// p_vc->pcm.bps / 8;
...
static bool _Read_Envelope(pxwrDoc *p_doc, pxtnVOICEUNIT *p_vc)
{
	...
	num = p_vc->envelope.head_num + p_vc->envelope.body_num + p_vc->envelope.tail_num;
	if (!_malloc_zero((void **)&p_vc->envelope.points, sizeof(sPOINT) * num))
		goto End;
	for (i = 0; i < num; i++) {
		if (!p_doc->v_r((s32 *)&p_vc->envelope.points[i].x))
			goto End;
		if (!p_doc->v_r((s32 *)&p_vc->envelope.points[i].y))
			goto End;
	}
...

{
	...
	if (!p_doc->w(_code, 1, 8))
		goto End;
	if (!p_doc->w(&_version, sizeof(u32), 1))
		goto End;
	if (!p_doc->w(&total, sizeof(s32), 1))
		goto End;

	work = 0;

	// p_ptv-> (5)
	if (!p_doc->v_w(work, &total))
		goto End; // basic_key (no use)
	if (!p_doc->v_w(work, &total))
		goto End;
	if (!p_doc->v_w(work, &total))
		goto End;
	if (!p_doc->v_w(_voice_num, &total))
		goto End;
	...
	for (v = 0; v < _voice_num; v++) {
		...
		if (!p_doc->v_w(p_vc->basic_key, &total))
			goto End;
		if (!p_doc->v_w(p_vc->volume, &total))
			goto End;
		if (!p_doc->v_w(p_vc->pan, &total))
			goto End;
		memcpy(&work, &p_vc->correct, sizeof(4));
		if (!p_doc->v_w(work, &total))
			goto End;
		if (!p_doc->v_w(p_vc->voice_flags, &total))
			goto End;
		if (!p_doc->v_w(p_vc->data_flags, &total))
			goto End;
	...
	// total size
	if (!p_doc->Seek(SEEK_CUR, -(total + 4)))
		goto End;
	if (!p_doc->w(&total, sizeof(s32), 1))
		goto End;
	if (!p_doc->Seek(SEEK_CUR, (total)))
		goto End;
...
bool pxtnWoice::PTV_Read(pxwrDoc *p_doc, bool *pb_new_fmt)
{
	...
	if (!p_doc->r(code, 1, 8))
		goto End;
	if (!p_doc->r(&version, sizeof(s32), 1))
		goto End;
	if (memcmp(code, _code, 8))
		goto End;
	if (!p_doc->r(&total, sizeof(s32), 1))
		goto End;
	...
	for (s32 v = 0; v < _voice_num; v++) {
		...
		if (!p_doc->v_r((s32 *)&p_vc->voice_flags))
			goto End;
		if (!p_doc->v_r((s32 *)&p_vc->data_flags))
			goto End;
		...
		if (p_vc->data_flags & PTV_DATAFLAG_WAVE) {
			if (!_Read_Wave(p_doc, p_vc))
				goto End;
		}
		if (p_vc->data_flags & PTV_DATAFLAG_ENVELOPE) {
			if (!_Read_Envelope(p_doc, p_vc))
				goto End;
		}
	}
	_type = pxtnWOICE_PTV;
